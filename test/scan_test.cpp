...
int main(int argc, char *argv[]) {
    ...
    // Scan
    data::BarcodeData result = scan::scanBarcode(processed_img, config.barcode_length_, config.skip_line_, config.fill_window_size_, config.min_barcode_size_);
    ...
    // Found number
    cout << "Barcode Number: " << result.barcode_number_ << " " <<  endl;

    // Draw ractangle
    cv::rectangle(img, result.p1_, result.p2_, cv::Scalar(0, 255, 0), 3);
